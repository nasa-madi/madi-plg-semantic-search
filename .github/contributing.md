...
Thank you for contributing to this MADI plugin! :heart: :tada:
...
We take security very seriously. We welcome any peer review of our 100% open source code to ensure nobody's app is ever compromised or hacked. As a web application developer you are responsible for any security breaches. We do our very best to make sure our code is as secure as possible by default.
...
Tests are located in the `test/` folder and can be run using the `npm run mocha` or `npm test` (with ESLint and code coverage) command.
